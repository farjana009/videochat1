...
//    link.setAttribute('href', 'https://webtel.dev.jacos.jp/rtc/r/'+this.params_.roomId);
//    document.head.appendChild(link);
    //document.querySelector("link[rel='canonical']").setAttribute("href","https://webtel.dev.jacos.jp/rtc/r/"+this.params_.roomId);
  return new Promise(function(resolve, reject) {
    if (!this.params_.roomId) {
...
//console.log();die();
    sendAsyncUrlRequest('POST', path).then(function(response) {
      ...
      if (!responseObj) {
        reject(Error('Error parsing response JSON333.'));
        return;
      }
      ...
      reject(Error('path::: '+path+' Failed to join the room: ' + error.message));
      return;
    }.bind(this));
  }.bind(this));
};
